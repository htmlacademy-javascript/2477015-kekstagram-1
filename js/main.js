...
const createMessage = () =>
  Array.from({length: 1}, () =>
    getRandomArrElement (ALL_COMMENTS));
...
const createDescriptionPhoto = () => ({
  ...
  comments: Array.from({length: getRandomInteger(1, NUMBER_COMMENTS)}, createComment),
});


const allDescriptionPhoto = () =>
  Array
    .from({length: ALL_DESCRIPTION_PHOTOS})
    .map((_, index) => createDescriptionPhoto (index + 1));
